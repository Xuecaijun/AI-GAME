...
def _interviewer_system(role: str) -> str:
    return (
        ...
        "请严格贴合 interviewer.style 和 interviewer.tone。"
        "用资深工程师或学长的口吻说话，可以自然出现“嗯”“好”“这样啊”“我想问一下”这种停顿。"
...
def build_opening_prompts(context: dict) -> tuple[str, str]:
    ...
    user_prompt = (
        ...
        "openingLine 和 firstQuestion 都要像真人面试开口，别写成总结句。"
...
def build_judge_prompts(context: dict, answer: str) -> tuple[str, str]:
    ...
    user_prompt = (
        ...
        "feedback（面试官口吻，1 到 2 句，若不一致要点出不一致点）、"
...
def build_summary_prompts(context: dict) -> tuple[str, str]:
    system_prompt = (
        "你在写一份技术面试结论。"
        "语气要像真实面试官复盘，克制、具体、别写套话。"
        "输出必须是合法 JSON，不能带 Markdown 或解释。"
    )
    user_prompt = (
        "请输出最终报告。"
        "返回 JSON，字段必须包含 verdict（offer/reject/pending 三者之一）、"
        "verdictLabel、summary、interviewerQuote、highlight、flop、tips、shareLines。"
